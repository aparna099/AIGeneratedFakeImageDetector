    ...
    if uploaded_files is not None:
        ...
        for uploaded_file in uploaded_files:
            ...
            if lap_var < threshold_lap_var:
                fake_reasons.append("The image lack sharpness and appears blurry and out of focus. Thus the image could have been intentionally tampered with or altered, suggesting it is more likely a  generated images.")
            else:
                real_reasons.append("The image is sharp and well-focused, with clear details and crisp edges, indicating it is likely a real image.")

            if entropy > threshold_entropy:
                fake_reasons.append("The image is less complex and less information-rich image and have repetitive patterns or uniform areas, which is often associated with generated images.")
            else:
                real_reasons.append("The image is a natural image with a lot of details and diverse patterns, suggesting it is more likely a real image.")

            if mean_gradient < threshold_mean_gradient:
                fake_reasons.append("The image has a smoother image with fewer visible edges or texture detail, which is often observed in generated images.")
            else:
                real_reasons.append("The image has a greater presence of edges or texture details in the image, characteristic of real images.")

            if color_consistency < threshold_color_consistency:
                fake_reasons.append("The image has color variations or alterations in different areas of the image, which is commonly observed in generated images.")
            else:
                real_reasons.append("The colors in the image are uniform and consistent throughout, indicating it is more likely a real image.")

            if light_misalignment > threshold_light_misalignment:
                fake_reasons.append("The lighting in the image appears to be inconsistent or misaligned, which is often a characteristic of generated images.")
            else:
                real_reasons.append("The lighting in the image appears to be consistent and properly aligned, suggesting it is more likely a real image.")
            ...
            if classification_result == 'Real':
                cv_reasons.append(real_reasons)
            else:
                cv_reasons.append(fake_reasons)
        ...
        if num_images > 0:
            ...
            with col1:
                if(num_images == 1):
                    ...
                    current_image_name = image_names[0]
                else:
                    ...
                    if(num_images > 1):
                        current_image_name = image_names[position]
                        dbreasons = cv_reasons[position]
                        message = prediction_messages[position]
            ...
            with col2:
                ...
                st.markdown(f"<p style='text-align: center; font-size: 22px;'>{prediction_messages[position]}</p>", unsafe_allow_html=True)
                ...
                for reason in cv_reasons[position]:
                ...
                with subcol1:
                    ...
                    if st.button("Add to Database"):
                        ...
                        dbsuccess = insert_data(connection, current_image_name, message, dbreasons)
                with subcol2:
                    if (dbsuccess):
...
def create_connection():
    ...
    try:
        connection = mysql.connector.connect(
            host='localhost',
            database='ImageAuthenticator',
            user='root',
            password='Aparn@080499',
...
def insert_data(connection, image_name, prediction_result, reasons):
    try:
        cursor = connection.cursor()

        # Retrieve the maximum primary key value
        cursor.execute("SELECT MAX(image_id) FROM HistoryLog")
        result = cursor.fetchone()
        last_pk = result[0] if result[0] is not None else 0

        # Increment the primary key value for the new insertion
        image_id = str(last_pk + 1)

        reasons_str = ', '.join(reasons)
        query = "INSERT INTO HistoryLog (image_id, image_name, prediction_result, reasons) VALUES (%s, %s, %s, %s)"
        values = (image_id, image_name, prediction_result, reasons_str)
